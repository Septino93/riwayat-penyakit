...
const prevBtn=document.getElementById('prevBtn'),nextBtn=document.getElementById('nextBtn'),submitBtn=document.getElementById('submitBtn'),errorBox=document.getElementById('errorBox');
...
function update(){
  ...
  if(!visible.includes(current))current=visible.find(i=>i>current)??visible[visible.length-1];
  steps.forEach((s,i)=>s.classList.toggle('active',i===current));
  const position=visible.indexOf(current)+1;const pct=Math.round((position/visible.length)*100);
  stepLabel.textContent=`Langkah ${position} dari ${visible.length}`;progressPercent.textContent=`${pct}%`;progressBar.style.width=`${pct}%`;
  prevBtn.hidden=position===1;nextBtn.hidden=position===visible.length;submitBtn.hidden=position!==visible.length;errorBox.hidden=true;
  if(position===visible.length)buildReview();window.scrollTo({top:0,behavior:'smooth'});
}
